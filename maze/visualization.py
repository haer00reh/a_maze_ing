...
def maze_drawer(stdscr, gen, scheme_name='color 1'):
            ...
            if is_path:
                ...
                for neighbor in cell.neigbours:
                    if getattr(neighbor, 'is_path', False):
                        if neighbor.y < cell.y and not (cell.val & 1):  #morth
                            path_north = True
                        elif neighbor.x > cell.x and not (cell.val & 2):  #east
                            path_east = True
                        elif neighbor.y > cell.y and not (cell.val & 4):  #south
                            path_south = True
                        elif neighbor.x < cell.x and not (cell.val & 8):  #west
                            path_west = True
                
                if path_north and path_south and not path_east and not path_west:
                    char = '┃'
                elif path_east and path_west and not path_north and not path_south:
                    char = '━'
    ...
    controls_text = f"[1-8] Color: {scheme_name}  [B] BFS  [D] DFS  [S] Solve  [R] Reset  [Q] Quit"
    menu_x = 2
...
def run_visualizer(stdscr, gen):
    ...
    while True:
        ...
        if key in [ord('1'), ord('2'), ord('3'), ord('4'), ord('5'), ord('6'), ord('7'), ord('8')]:
            ...
            gen.reset_solver()
            gen.solve_dfs(stdscr, maze_drawer, scheme_names[current_scheme_idx])
        elif key in [ord('r')]:
            gen.reset_maze()
        elif key in [ord('q')]:
            break
