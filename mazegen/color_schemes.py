import curses
from typing import Dict, Tuple

COLOR_SCHEMES: Dict[str, Dict[str, Tuple[int, int]]] = {
    'color 1': {
        'entry': (curses.COLOR_RED, curses.COLOR_BLACK),
        'exit': (curses.COLOR_WHITE, curses.COLOR_BLACK),
        'solution': (curses.COLOR_RED, curses.COLOR_BLACK),
        'walls': (curses.COLOR_WHITE, curses.COLOR_BLACK),
        'title': (curses.COLOR_RED, curses.COLOR_BLACK),
    },
    'color 2': {
        'entry': (curses.COLOR_CYAN, curses.COLOR_BLACK),
        'exit': (curses.COLOR_BLUE, curses.COLOR_BLACK),
        'solution': (curses.COLOR_CYAN, curses.COLOR_BLACK),
        'walls': (curses.COLOR_BLUE, curses.COLOR_BLACK),
        'title': (curses.COLOR_CYAN, curses.COLOR_BLACK),
    },
    'color 3': {
        'entry': (curses.COLOR_MAGENTA, curses.COLOR_BLACK),
        'exit': (curses.COLOR_WHITE, curses.COLOR_BLACK),
        'solution': (curses.COLOR_WHITE, curses.COLOR_BLACK),
        'walls': (curses.COLOR_MAGENTA, curses.COLOR_BLACK),
        'title': (curses.COLOR_MAGENTA, curses.COLOR_BLACK),
    },
    'color 4': {
        'entry': (curses.COLOR_GREEN, curses.COLOR_BLACK),
        'exit': (curses.COLOR_YELLOW, curses.COLOR_BLACK),
        'solution': (curses.COLOR_GREEN, curses.COLOR_BLACK),
        'walls': (curses.COLOR_YELLOW, curses.COLOR_BLACK),
        'title': (curses.COLOR_GREEN, curses.COLOR_BLACK),
    },
    'color 5': {
        'entry': (curses.COLOR_BLUE, curses.COLOR_BLACK),
        'exit': (curses.COLOR_CYAN, curses.COLOR_BLACK),
        'solution': (curses.COLOR_BLUE, curses.COLOR_BLACK),
        'walls': (curses.COLOR_CYAN, curses.COLOR_BLACK),
        'title': (curses.COLOR_BLUE, curses.COLOR_BLACK),
    },
    'color 6': {
        'entry': (curses.COLOR_RED, curses.COLOR_BLACK),
        'exit': (curses.COLOR_YELLOW, curses.COLOR_BLACK),
        'solution': (curses.COLOR_RED, curses.COLOR_BLACK),
        'walls': (curses.COLOR_YELLOW, curses.COLOR_BLACK),
        'title': (curses.COLOR_YELLOW, curses.COLOR_BLACK),
    },
    'color 7': {
        'entry': (curses.COLOR_MAGENTA, curses.COLOR_BLACK),
        'exit': (curses.COLOR_GREEN, curses.COLOR_BLACK),
        'solution': (curses.COLOR_MAGENTA, curses.COLOR_BLACK),
        'walls': (curses.COLOR_GREEN, curses.COLOR_BLACK),
        'title': (curses.COLOR_MAGENTA, curses.COLOR_BLACK),
    },
    'color 8': {
        'entry': (curses.COLOR_WHITE, curses.COLOR_BLACK),
        'exit': (curses.COLOR_CYAN, curses.COLOR_BLACK),
        'solution': (curses.COLOR_WHITE, curses.COLOR_BLACK),
        'walls': (curses.COLOR_CYAN, curses.COLOR_BLACK),
        'title': (curses.COLOR_WHITE, curses.COLOR_BLACK),
    },
}
